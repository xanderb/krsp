...
class Controller_Admin_Decree extends Controller_Back implements Controller_Admin_Crud {
    ...
    public function action_index()
    {
        ...
        $decree = ORM::factory('decree')
            ...
            ->find_all();
        $table_view->datas = $decree;
        $table_view->badges = $this->badges;
        $table_view->total_items = array($total_items);

        if(ceil($total_items / $this->config->items_per_page) > 1){
            $table_view->paginator = Help::render_paginator('decree', $this->back_menu[0]['href'], $page, $total_items); //добавление постраничной навигации
        }


        $admin_view = View::factory('/back/control_panel');
        $admin_view->p_title = 'Решения по материалам дел'; //заголовок выводимый на страницу в h1
    ...
    public function action_add()
    {
        ...
        if(isset($_POST['text']))
        {
            ...
            try
            {
                ...
                $content->back_path_text = 'Вернуться назад к решениям';
            }
            catch (ORM_Validation_Exception $e)
            {
                $content = View::factory('/back/edit_decree');
                $content->sub_menus = $this->back_menu;
                $content->legend = "Форма добавления решения";
                $content->data = $decree;
        ...
        $admin_view = View::factory('/back/control_panel');
        $admin_view->p_title = 'Добавление решения';
    ...
    public function action_edit()
    {
        ...
        $admin_view = View::factory('/back/control_panel');
        $admin_view->p_title = 'Редактирование решения';
        $admin_view->content = $content;
        $this->template->body = $admin_view;
    }
    ...
    public function action_delete()
    {
        ...
        if(isset($_POST['id'])){
            ...
            try{
                ...
                $content->back_path = '/admin/decree';
                $content->back_path_text = 'Вернуться назад к списку решений';
            }catch(Exception $e){
                $content = View::factory('/back/error');
        ...
        $admin_view = View::factory('/back/control_panel');
        $admin_view->p_title = 'Удаление решения';
        $admin_view->content = $content;
        $this->template->body = $admin_view;
    }
}